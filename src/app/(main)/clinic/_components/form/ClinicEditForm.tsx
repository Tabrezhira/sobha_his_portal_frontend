...
interface ClinicEditFormProps {
  ...
  clinicSaved?: boolean
  mode?: "create" | "edit"
  initialData?: ClinicEditFormInitialData
  hideActions?: boolean
}

export type ClinicEditFormRef = {
  getPayload: () => Record<string, unknown>
  isValid: () => boolean
}

export type ClinicEditFormInitialData = Partial<ClinicVisit> & {
  _id?: string
  id?: string
  slNo?: string | number
}

const ClinicEditForm = forwardRef<ClinicEditFormRef, ClinicEditFormProps>(
  function ClinicEditForm(
    ...
    const [form, setForm] = useState(() => {
      ...
      return {
        ...
        slNo: initialData.slNo !== undefined && initialData.slNo !== null ? String(initialData.slNo) : generateSlNo(),
        date: toDateInput(initialData.date) || getLocalDate(),
        time: initialData.time ?? getLocalTime(),
        ...
        totalSickLeaveDays: initialData.totalSickLeaveDays !== undefined && initialData.totalSickLeaveDays !== null ? String(initialData.totalSickLeaveDays) : "",
    ...
    // Only update slNo, date, time if empty on mount (mostly for create mode)
    useEffect(() => {
      setForm((prev) => ({
        ...prev,
    ...
    const [medicines, setMedicines] = useState(() => {
      if (!initialData?.medicines?.length) return [emptyMedicine]
      return initialData.medicines.map((item) => ({
        name: item.name ?? "",
    ...
    const [secondaryDiagnoses, setSecondaryDiagnoses] = useState<string[]>(() => {
      return initialData?.secondaryDiagnosis?.length ? initialData.secondaryDiagnosis : []
    })
    const [nurseAssessments, setNurseAssessments] = useState<string[]>(() => {
      return initialData?.nurseAssessment?.length ? initialData.nurseAssessment : []
    })
    const [referralDetails, setReferralDetails] = useState(() => {
      if (!initialData) return emptyReferral
      return {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        if (isEditMode && clinicRecordId) {
          ...
          const updateResponse = await fetch(`${baseUrl}/clinic/${clinicRecordId}`, {
            method: "PUT",
            headers: {
          ...
          if (!updateResponse.ok) {
            throw new Error("Failed to update clinic visit")
          }
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    <ScrollArea className="h-[200px]">
                      {getDisplayOptions(
                        ["Approved", "Not Approved", "SL Approved", "SL Not Approved"],
                        form.sickLeaveStatus,
                      ).map((option) => (
                        <SelectItem key={option} value={option}>
                          {option}
                        </SelectItem>
