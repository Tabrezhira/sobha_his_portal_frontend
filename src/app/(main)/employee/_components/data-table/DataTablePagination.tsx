...
export function DataTablePagination<TData>({
  ...
  const fallbackTotal = table.getFilteredRowModel().rows.length
  const total = typeof totalRows === "number" ? totalRows : fallbackTotal
  const currentPage = table.getState().pagination.pageIndex
  const firstRowIndex = total === 0 ? 0 : currentPage * pageSize + 1
  const lastRowIndex = total === 0 ? 0 : Math.min(total, firstRowIndex + pageSize - 1)
