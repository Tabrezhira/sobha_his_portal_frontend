...
const SuggestionInput = ({
  ...
  const { items, loading } = useDropdownSearch(baseUrl, category, safeValue)
...
const HospitalEditForm = forwardRef<HospitalEditFormRef, HospitalEditFormProps>(
  function HospitalEditForm(
    ...
    const [form, setForm] = useState({
      ...
      dod: "",
      noOfDaysHospitalized: "",
      fitnessStatus: "",
      isolationOrRehabilitationRequired: false,
      remarks: "",
      createdBy: "",
    })
    ...
    useEffect(() => {
      ...
      setForm((prev) => ({
        ...
        dod: toDateInput(initialData.dod),
        noOfDaysHospitalized:
          initialData.noOfDaysHospitalized !== undefined &&
            initialData.noOfDaysHospitalized !== null
            ? String(initialData.noOfDaysHospitalized)
            : "",
        fitnessStatus: initialData.fitnessStatus ?? "",
        isolationOrRehabilitationRequired: Boolean(initialData.isolationOrRehabilitationRequired),
      ...
      setFollowUp(
        initialData.followUps?.length
          ? initialData.followUps.map((item) => ({
            date: item.followUpDate ? String(item.followUpDate).slice(0, 10) : "",
    ...
    useEffect(() => {
      if (form.doa && form.dod) {
        const admission = new Date(form.doa)
        const discharge = new Date(form.dod)
        if (!isNaN(admission.getTime()) && !isNaN(discharge.getTime())) {
          ...
          if (diffDays >= 0) {
            setForm((prev) => {
              if (prev.noOfDaysHospitalized !== String(diffDays)) {
                return { ...prev, noOfDaysHospitalized: String(diffDays) }
              }
              return prev
            })
          }
        ...
        setForm((prev) => {
          if (prev.noOfDaysHospitalized !== "") {
            return { ...prev, noOfDaysHospitalized: "" }
          }
          return prev
        })
      }
    ...
    const canSubmit = useMemo(() => {
      return form.empNo && form.name && form.emiratesId
    }, [form])
    ...
    const buildPayload = (): UpdateIpAdmissionInput => {
      const toNumber = (value: string) => (value ? Number(value) : undefined)

      const filteredFollowUps = followUp
        .filter((item) => item.date || item.remarks)
        .map((item) => ({
          followUpDate: item.date ? new Date(item.date) : undefined,
      ...
      return {
        ...
        doa: form.doa ? new Date(form.doa) : undefined,
        ...
        secondaryDiagnosis: secondaryDiagnosis || undefined,
        status: form.status || undefined,
        dischargeSummaryReceived: form.dischargeSummaryReceived,
        dod: form.dod ? new Date(form.dod) : undefined,
        noOfDaysHospitalized: toNumber(form.noOfDaysHospitalized),
        followUps: filteredFollowUps.length ? filteredFollowUps : undefined,
        fitnessStatus: form.fitnessStatus || undefined,
        isolationOrRehabilitationRequired: form.isolationOrRehabilitationRequired,
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      if (form.doa && form.dod && new Date(form.dod) < new Date(form.doa)) {
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="col-span-2 lg:col-span-3">
                <SuggestionInput
                  ...
                  onChange={(value) => setSecondaryDiagnosis(value)}
                  category={dropdownCategories.primaryDiagnosis}
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div>
                ...
                <Input
                  ...
                  value={form.dod}
                  onChange={(e) =>
                    updateForm("dod", e.target.value)
                  }
                />
              </div>
              <div>
                <Label htmlFor="noOfDaysHospitalized" className="font-medium">
                  Days Hospitalized
                </Label>
                <Input
                  id="noOfDaysHospitalized"
                  type="number"
                  disabled
                  enableStepper={false}
                  className="mt-2"
                  value={form.noOfDaysHospitalized}
                  onChange={(e) =>
                    updateForm("noOfDaysHospitalized", e.target.value)
                  }
                />
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    updateForm("isolationOrRehabilitationRequired", value === "Yes")
                  }
                >
                  <SelectTrigger className="mt-2">
                    <SelectValue placeholder="Select option" />
